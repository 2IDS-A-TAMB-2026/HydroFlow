...
class _RelatorioplantasPageState extends State<RelatorioplantasPage> {
  ...
  Future<void> consultarplantas() async {
    try{
      ...
      final resposta = await http.get(
        Uri.parse(
          'http://desktop-38ilvp3/HydroFlow/public/api/plantas' //MUDAR ESSA BOMBA AQUIIIIIIIIIIIIIIIIIIII
        ),
  ...
  Future<void> excluirplantas(dynamic plantasID) async {
    try{
      ...
      final resposta = await http.delete(
        Uri.parse(
          'http://desktop-38ilvp3/HydroFlow/public/api/$plantasID' //MUDAR ESSA BAGAÇA
        ),
        //informar para a API que os dados são em json
        headers: {
        'Accept': 'application/json',
...
class _RelatorioplantasPageState extends State<RelatorioplantasPage> {
  ...
  Future<void> excluirplantas(dynamic plantasID) async {
    try{
      ...
      final resposta = await http.delete(
        Uri.parse(
          'http://DESKTOP-38ILVP3/HydroFlow/public/api/plantas/$plantasID' //MUDAR ESSA BAGAÇA
        ),
        //informar para a API que os dados são em json
        headers: {
        'Accept': 'application/json',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: bgPage,
      appBar: AppBar(
        title: Text('Relatório de Plantas', style: TextStyle(fontSize: 20 * f)),
        backgroundColor: appBarBg,
        foregroundColor: Colors.white,
        elevation: 0,
                                ...
                                final confirmar = await showDialog<bool>(
                                  ...
                                  builder: (context){
                                    return AlertDialog(
                                      ...
                                      content: Text(
                                        "Deseja excluir o modelo ${plantas['NOME']}",
