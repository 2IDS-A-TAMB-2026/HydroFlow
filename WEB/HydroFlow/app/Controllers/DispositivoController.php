...
class Dados_SensoresController extends BaseController
{
    ...
    public function index()
    {
        ...
        // 2. Captura os valores dos filtros vindos da URL (limpando strings vazias)
        $dataInicial  = $this->request->getGet('data_inicial') ?: null;
        $dataFinal    = $this->request->getGet('data_final') ?: null;
        $tempMin      = $this->request->getGet('temp_min') !== '' ? $this->request->getGet('temp_min') : null;
        $umidadeMax   = $this->request->getGet('umidade_max') !== '' ? $this->request->getGet('umidade_max') : null;
        $statusFiltro = $this->request->getGet('status_filtro') ?: 'todos';

        // Instancia a conexão nativa com o banco de dados
        $db = \Config\Database::connect();

        // 3. Busca os Sensores pertencentes ao usuário logado
        ...
        // Se o usuário não tiver sensores cadastrados
        ...
        // --- AGRUPAMENTO EM MÉDIA DIÁRIA PARA O GRÁFICO (ORDEM CRONOLÓGICA ROBUSTA) ---
        $agrupadoPorDia = [];
        foreach ($medicoesFiltradas as $med) {
            $dataIso = $med['DDS_DATA']; // ex: '2022-12-12'
            if (!isset($agrupadoPorDia[$dataIso])) {
                $agrupadoPorDia[$dataIso] = ['temp_soma' => 0, 'umid_soma' => 0, 'qtd' => 0];
            }
            $agrupadoPorDia[$dataIso]['temp_soma'] += (float)($med['DDS_TEMP'] ?? 0);
            $agrupadoPorDia[$dataIso]['umid_soma'] += (float)($med['DDS_UMIDADE'] ?? 0);
            $agrupadoPorDia[$dataIso]['qtd']++;
        }

        // Ordena por data (Antigo -> Recente)
        ksort($agrupadoPorDia);

        $labels = [];
        $temperaturas = [];
        $umidades = [];

        foreach ($agrupadoPorDia as $dataIso => $valores) {
            $labels[]       = date('d/m', strtotime($dataIso));
    ...
    public function receber()
    {
        ...
        $novaMedicao = [
            ...
            'DDS_UMIDADE_SOLO' => $json['dds_umidade_solo'] ?? null, // CORRIGIDO: Evita Notice se ausente
        ...
        if ($this->dadosSensoresModel->insert($novaMedicao)) {
            return $this->response->setJSON([
                'status'  => 'success',
                'message' => 'Medição de temperatura e umidade gravada com sucesso!'
            ])->setStatusCode(201);
        } else {
            return $this->response->setJSON([
                'status'  => 'error',
