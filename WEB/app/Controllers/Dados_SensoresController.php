...
class dados_SensoresController extends BaseController
{
    ...
    public function index()
    {
        ...
        // 7. Executa a busca ordenando para a tabela (mais recente primeiro)
        ...
        // --- LÓGICA DO GRÁFICO (MÉDIA DIÁRIA OU MEDIÇÕES DO DIA POR HORA) ---

        // Verifica se a busca foi delimitada para um único dia específico
        $isMesmoDia = (!empty($dataInicial) && !empty($dataFinal) && $dataInicial === $dataFinal);
        ...
        if ($isMesmoDia) {
            // CASO 1: Um único dia selecionado -> Exibe todas as medições ordenadas por HORA
            // Revertemos para ficar em ordem cronológica no gráfico (do horário inicial ao final)
            $medicoesOrdenadas = array_reverse($medicoesFiltradas);

            foreach ($medicoesOrdenadas as $med) {
                $labels[]       = date('H:i', strtotime($med['DDS_HORA']));
                $temperaturas[] = round((float)($med['DDS_TEMP'] ?? 0), 1);
                $umidades[]     = round((float)($med['DDS_UMIDADE'] ?? 0), 1);
            }

            $tituloGrafico = "Comportamento do Ambiente (Detalhamento por Hora)";
        } else {
            // CASO 2: Período de vários dias ou sem filtro de data -> Calcula Média Diária
            $agrupadoPorDia = [];
            ...
            $tituloGrafico = "Comportamento do Ambiente (Médias Diárias)";
        }

        $data['titulo_grafico'] = $tituloGrafico;
        $data['dados_grafico']  = [
            'labels'       => $labels,
    ...
    public function receber()
    {
        ...
        $novaMedicao = [
            ...
            'DDS_UMIDADE_SOLO' => $json['dds_umidade_solo'] ?? null,
        ...
        if ($this->dadosSensoresModel->insert($novaMedicao)) {
            return $this->response->setJSON([
                'status'  => 'success',
                'message' => 'Medição de temperatura e umidade gravada!'
            ])->setStatusCode(201);
        } else {
            return $this->response->setJSON([
                'status'  => 'error',
